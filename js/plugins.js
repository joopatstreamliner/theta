...
$(window).bind('load', function () {
	...
	function parallaxInit() {
		...
		if (testMobile == null)
		{
			$('.header_foto.jpg').parallax("50%", 0.5);
...
window.onload = function () {
			...
			var latlng = new google.maps.LatLng(51.432073,5.5241383);
			...
			var myOptions = {
			 ...
			 var contentString = '<div id="content">'+
			  '<div id="siteNotice">'+
			  '</div>'+
			  '<h4>E.S.R. Th&ecirc;ta</h4>'+
			  '<p>Kanaaldijk Zuid 50</p>' +
			  '<p>5645 TA, Eindhoven</p>'+
			  '</div>';
		  ...
		  var infowindow = new google.maps.InfoWindow({
			...
			var myLatlng = new google.maps.LatLng(51.432062, 5.526309);
		...
		$('form#contact-us').submit(function() {
			...
			$('.requiredField').each(function() {
				if($.trim($(this).val()) == '') {
					...
					$(this).parent().append('<span class="error">Verplicht '+labelText+' in te vullen.</span>');
					...
					if(!emailReg.test($.trim($(this).val()))) {
						var labelText = $(this).prev('label').text();
						$(this).parent().append('<span class="error">Sorry! Je hebt een verkeerd '+labelText+' ingeuvld.</span>');
						$(this).addClass('inputError');
						hasError = true;
					}
				}
			...
			if(!hasError) {
				...
				$.post($(this).attr('action'),formInput, function(data){
					$('form#contact-us').fadeOut("slow", function() {				   
						$(this).before('<div class="mail-message"><p class="mail-head">Bedankt!</p><p>Je email is verzonden!</p></div>');
					});
				});
			}
